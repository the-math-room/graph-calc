...
- Keep graph rendering responsive by separating sampling from painting. Workspace compilation produces semantic plots, sampling turns them into drawing data for a specific viewport, and the canvas layer paints that data without evaluating math.
- Treat workspace plots and sampled plots as different kinds of values. Workspace plots describe graphable semantics in world coordinates; sampled plots are viewport-specific screen-space render artifacts. Non-workspace modules should depend on `workspace-sampling.ts`, not on individual sampling implementations.
- Treat saved workspaces as versioned documents. A workspace export preserves user-authored row state such as source text, editor LaTeX, row mode, and view preferences; evaluation results and plots are derived from those rows after import.
...
- Parse structure before assigning meaning. Shared helpers such as `findTopLevelOperator` should identify top-level operators while respecting delimiters; the syntax or workspace layer that calls them decides whether that operator means assignment, comparison, a parametric bound, or something else.
...
- Do not auto-plot case definitions. A row like `a(n) = a(n-1) + a(n-2)` defines a function, but sampling it as a continuous curve is both mathematically misleading and computationally dangerous. Plot explicit values or expressions derived from the function instead.
...
- Workspace import/export uses `lambda-graph.workspace.v1`. Import rejects unsupported versions and malformed row documents instead of guessing; future migrations should be explicit version readers.
